# P&ID Pipeline Application — План разработки

> **Версия:** 2.0 (обновлён 09.02.2026)
> **Архитектура:** Monorepo, unified docker-compose (P&ID + CVAT)
...
| Phase 2 | YOLO + CVAT (Backend + UI) | ✅ Завершена |
| Рефакторинг | Этапы 0-4 | ✅ Завершён (09.02.2026) |
| Phase 3 | Сегментация + Скелетизация | ⬜ Следующая |
...
| Docker Compose | Unified (P&ID + CVAT) | Единый запуск, общая сеть |
| Status updates | HTTP Polling (MVP) | Проще, заменяемый на WebSocket потом |
| CVAT Client | Sync + singleton | Connection pooling, `asyncio.to_thread()` в API |
| UI Architecture | Thin MainWindow + виджеты | MainWindow=координатор (287 строк), логика в DiagramListWidget |
| File download | Через API endpoint | UI не обращается к filesystem напрямую |
| Logging | Единый `app/core/logging.py` | ColoredFormatter, LOG_LEVEL из .env |
...
### 2.4 PySide6 UI для YOLO блока - изменили визуал. Нужно переработаь для более удачного. Добавлен основной функционал
...
def task_segment_pipes(self, diagram_uid: str):
    ...
    4. Сохранить pipe_mask.png + node_mask.png
...
| Phase | Описание | Часы | Статус |
|-------|----------|------|--------|
| 1 | Инфраструктура | 6-7 | ✅ |
| 2 | YOLO + CVAT | 22 | ✅ |
| Рефакторинг | Этапы 0-4 | ~6 | ✅ |
...
| | |
|---|---|
| **Было в плане** | Phase 2 — WebSocket для real-time обновлений |
| **Решение** | Отложить до Phase 6 |
| **Причина** | HTTP polling каждые 2 сек достаточен для MVP. Операции длятся минуты, задержка не критична. WebSocket требует reconnect logic и state sync. |
| **Когда делать** | Phase 6, или раньше если появятся короткие операции (<5 сек) |
| **Как сделать** | Создать `WebSocketProvider` с тем же интерфейсом, что у `StatusProvider` |

### Batch processing

| | |
|---|---|
| **Было в плане** | Phase 6 — загрузка папки с диаграммами |
| **Решение** | Отложить |
| **Причина** | Текущий workflow — по одной диаграмме. Batch не приоритет для MVP. |
| **Когда делать** | Phase 6, после стабилизации single-diagram workflow |

### E2E тесты

| | |
|---|---|
| **Было в плане** | Phase 6 — полные end-to-end тесты |
| **Решение** | Отложить |
| **Причина** | Нужна работающая YOLO модель и CVAT instance. Unit тесты можно раньше. |
| **Когда делать** | Phase 6, после завершения ML этапов |

### CI/CD

| | |
|---|---|
| **Было в плане** | Phase 6 — GitHub Actions |
| **Решение** | Отложить |
| **Причина** | Проект в активной разработке, развёртывание ручное |
| **Когда делать** | Phase 6 |

### Async CVAT Client

| | |
|---|---|
| **Было в плане** | Phase 2 — переписать CVATClient на httpx.AsyncClient |
| **Решение** | Заменено на singleton с connection pooling |
| **Причина** | Sync client + `asyncio.to_thread()` проще и надёжнее. Async client требует переписать все методы и тесты. Выигрыш минимален — CVAT вызовы редки. |
| **Когда делать** | Не планируется. Текущее решение достаточно. |
...
**Phase 3: Сегментация + Скелетизация** — интеграция U2-Net++ модуля, Celery task для сегментации, API endpoints.
